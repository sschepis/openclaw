...
function flattenSchema(
  ...
  if (type === "object" && schema.properties) {
    ...
    for (const [key, propSchema] of sorted) {
      ...
      const nested = flattenSchema(propSchema, propValue, hints, [...path, key], depth + 1);
      results.push(...nested);
    }
  } else {
    // Leaf property
...
function searchProperties(definitions: PropertyDefinition[], query: string): SearchResult[] {
  if (!query.trim()) {
...
```typescript
function renderHighlightedPath(pathKey: string, ranges: [number, number][]): TemplateResult {
  if (ranges.length === 0) {
  ...
  for (const [start, end] of ranges.sort((a, b) => a[0] - b[0])) {
    ...
    parts.push(html`<mark class="property-path__match"> ${pathKey.slice(start, end)} </mark>`);
    lastEnd = end;
  }
  ...
  return html`
    <div class="property-editor property-editor--string ${isModified ? "modified" : ""}">
      <input
        ...
        placeholder=${def.defaultValue !== undefined ? `Default: ${String(def.defaultValue)}` : ""}
        @input=${(e: Event) => {
...
function renderEnumEditor(ctx: PropertyRowContext): TemplateResult {
  ...
  return html`
    ...
    >
      ...
      ${options.map(
        (opt) => html`
          <option value=${String(opt)} ?selected=${opt === currentValue}>${String(opt)}</option>
        `,
      )}
    </select>
  `;
...
function renderArrayEditor(ctx: PropertyRowContext): TemplateResult {
  ...
  return html`
    <div class="property-editor property-editor--array ${isModified ? "modified" : ""}">
      <button type="button" class="property-editor__array-toggle" @click=${onExpand}>
